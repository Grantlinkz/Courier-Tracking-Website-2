...
(function( $ ) {
	...
	'if ( hide_total == "yes" ) {' +
		...
		'return;' +
	'}' +
	'jQuery( "#btSubtotalId" + id ).find( ".btQuoteSubtotalCalc" ).php( val );' +
	'};';
	...
	window.bt_cc_set_slider_value = function( elem, value, calc ){
		if ( elem.slider( 'instance' ) ) { 
			...
			elem.find('.btQuoteSliderValue').php( value );	
			if ( calc ) {
	...
	window.bt_cc_get_slider_value = function( elem ){
		if ( elem.slider( 'instance' ) ) {
			val = bt_parse_float( elem.find('.btQuoteSliderValue').php() );
		} else {
				...
				$( this ).find( '.btQuoteBookingForm' ).find( '.btQuoteMBlock' ).each(function() {
					...
					$( this ).find( '.btQuoteItem' ).each(function() {
						...
						$( this ).find( '.btQuoteText' ).each(function() {
							...
							if ( m_first ) {
								m_val = val;
								label = $( this ).closest( '.btQuoteItem' ).find( 'label' ).php();
							} else {
								m_total *= val;
							}
							m_first = false;
	...
	window.bt_quote_total = function( c ) {
		...
		c.find( '.btQuoteTotalCalc' ).php( total );
		c.data( 'total', total );
	}
	...
	window.bt_cc_init = function( ) {
		...
		$( ".btQuoteBooking" ).each(function( index ) {
			...
			c.find( '.btQuoteSlider' ).each(function() {
				$( this ).slider({
					...
					change: function( event, ui ) { 
						$( this ).find('.btQuoteSliderValue').php( window.rtl_direction * $( this ).slider( 'value' ) );
			...
			c.find( '.btQuoteSlider' ).each(function() {
				...
				$( this ).slider({
					slide: function( event, ui ) {
						...
						this_slider.find('.btQuoteSliderValue').php( val );
						bt_quote_total( c );
						bt_paypal_items( c );
					}
				});
			...
			c.find( '.btContactSubmit' ).click(function() {
				...
				if ( ! val ) {
					c.find( '.btSubmitMessage' ).hide().php( c.data( 'message_mandatory' ) ).fadeIn();
					return false;
				}
				...
				if ( email_gdpr == 'no' ) {
					c.find( '.btSubmitMessage' ).php(c.data( 'email_gdpr_not_text' ) );
					return false;
				}
				...
				c.find( '.btSubmitMessage' ).hide().php( c.data( 'message_please_wait' ) ).fadeIn();
					...
					success: function( response ) {
						...
						if ( response == 'ok' ) {
							if ( c.data( 'url_confirmation' ) == '' || c.data( 'url_confirmation' ) == null ) {
								c.find( '.btSubmitMessage' ).hide().php( c.data( 'message_success' ) ).fadeIn();
							} else {
								window.location = c.data( 'url_confirmation' );
							}
							
						} else {
							c.find( '.btSubmitMessage' ).hide().php( c.data( 'message_error' ) ).fadeIn();
						}
					},
